...
export const registerOrder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log("Numero de Guia" + trackingNumber);
        if (!userID) {
            res.status(401).json({ error: "Usuario no autenticado" });
            return;
        }
        const { tracking_number, recipient_name, recipient_address, package_description, weight } = req.body;
        if (!tracking_number || !recipient_name || !recipient_address || !package_description || !weight) {
